 ...
 *
 * [OK] NOTE: Just three short functions added below, each denoted by [OK].
 */
...
template <class Value, class SkipValue = size_t,
...
struct EliasFanoEncoderV2
{
    ...
    {
        if (begin == end) return MutableCompressedList();

        EliasFanoEncoderV2 encoder(size_t(end - begin), *(end - 1));
        for (; begin != end; ++begin) encoder.add(*begin);
    ...
    /* NOTE: [OK] */
    template <class RandomAccessIterator>
    static size_t encode_into_buffer(RandomAccessIterator begin, RandomAccessIterator end, ValueType upperBound,
                                     uint8_t *buf)
    {
        if (begin == end) exit(1);

        size_t             size = size_t(end - begin);
        EliasFanoEncoderV2 encoder(size, upperBound, buf);

        for (; begin != end; ++begin) encoder.add(*begin);

        size_t bytes     = Layout::fromUpperBoundAndSize(upperBound, size).bytes();
        size_t alignment = std::max(sizeof(ValueType), sizeof(SkipValueType));
    ...
    explicit EliasFanoEncoderV2(const MutableCompressedList &result)
        : lower_(result.lower),
          ...
          forwardPointers_(reinterpret_cast<SkipValueType *>(result.forwardPointers)),
          result_(result)
    {
    ...
    /* NOTE: [OK] */
    EliasFanoEncoderV2(size_t size, ValueType upperBound, uint8_t *buf)
        : EliasFanoEncoderV2(Layout::fromUpperBoundAndSize(upperBound, size).fromList(buf))
    {
    }
...
template <class Value, class SkipValue, size_t kSkipQuantum, size_t kForwardQuantum>
struct EliasFanoEncoderV2<Value, SkipValue, kSkipQuantum, kForwardQuantum>::Layout
{
    static Layout fromUpperBoundAndSize(size_t upperBound, size_t size)
    {
        ...
        const size_t upperSizeBits = (upperBound >> numLowerBits) +  // Number of 0-bits to be stored.
                                     size;                           // 1-bits.
...
class UpperBitsReader : ForwardPointers<Encoder::forwardQuantum>, SkipPointers<Encoder::skipQuantum>
{
   ...
   public:
    ...
    ValueType skip(SizeType n)
    {
        ...
        if (Encoder::forwardQuantum > 0 && n > Encoder::forwardQuantum)
        {
            ...
            const size_t dest =
                folly::loadUnaligned<SkipValueType>(this->forwardPointers_ + (steps - 1) * sizeof(SkipValueType));
    ...
    ValueType skipToNext(ValueType v)
    {
        ...
        if (Encoder::skipQuantum > 0 && v >= value_ + Encoder::skipQuantum)
        {
            ...
            const size_t dest =
                folly::loadUnaligned<SkipValueType>(this->skipPointers_ + (steps - 1) * sizeof(SkipValueType));
...
class EliasFanoReader
{
   public:
    ...
    explicit EliasFanoReader(const typename Encoder::CompressedList &list)
        : upper_(list), lower_(list.lower), size_(list.size), numLowerBits_(list.numLowerBits)
    {
        assert(Instructions::supported());
    ...
    ValueType previousValue() const
    {
        return readLowerPart(upper_.position() - 1) | (upper_.previousValue() << numLowerBits_);
    }
   ...
   private:
    ...
    constexpr static ValueType kInvalidValue = std::numeric_limits<ValueType>::max();
    ...
    ValueType readLowerPart(SizeType i) const
    {
        // if(i >= size_)
        // {
        //     printf("i = %lu, size_ = %lu, lastValue_ = %lu\n", i, size_, lastValue_);

        //     const size_t         pos  = i * numLowerBits_;
        //     const unsigned char *ptr  = lower_ + (pos / 8);
        //     const uint64_t       ptrv = folly::loadUnaligned<uint64_t>(ptr + 100lu*1000*1000*1000lu);
        ...
        if (numLowerBits_ >= sizeof(ValueType) * 8) __builtin_unreachable();
