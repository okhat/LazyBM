...
struct EliasFanoCompressedListBase
{
    ...
    size_t upperSize() const { return size_t(data.end() - upper); }
...
struct EliasFanoEncoderV2<Value, SkipValue, kSkipQuantum, kForwardQuantum>::Layout
{
    ...
    size_t bytes() const { return lower + upper + skipPointers + forwardPointers; }
...
class UpperBitsReader : ForwardPointers<Encoder::forwardQuantum>, SkipPointers<Encoder::skipQuantum>
{
   ...
   public:
    ...
    void setDone(SizeType endPos) { position_ = endPos; }
